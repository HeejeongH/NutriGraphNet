...
**Status:** Draft v2.2 — 2026-07-18 (**HFRS-DA 실측 반영 + 정직한 두-모델 프레임 확정**: HFRS-DA 5-fold 실행 완료(scripts/run_hfrsda_real.bat), Table 1/B/B-AUC/B-NDCG에 HFRS-DA 행·열 추가. **핵심 정정**: HFRS-DA가 AUC/NDCG/MRR에서 전 밀도 NutriGraphNet을 앞섬 → "NutriGraphNet is best of six" 주장 철회. 논지를 "보조 구조가 상호작용을 대체 — **두 모델(NutriGraphNet, HFRS-DA)이 독립 입증**"으로 전환. NutriGraphNet 고유 기여 = health-gradient 조절성(HFRS-DA는 고정) + 저밀도 최고 안정성. **§8.4 Finding H2 신규**: SOTA 5회 시도(cosine decoder/health-loss/over-smoothing/InfoNCE/content-anchor) 전부 HFRS-DA NDCG 0.625 미달, 체크포인트 진단으로 food 임베딩 11/64 유효차원 확인 → 협업 인코더의 retrieval-vs-ordering 구조적 한계로 결론(negative result를 분석 기여로). Abstract/Contribution 3/Conclusion 2/§11.6 정정, AUC 미해결 질문은 HFRS-DA도 동일 추세로 부분 해소. v2.1: **HFRS-DA 실제 baseline 추가**: 공식 구현(Forouzandeh et al., 2024) 기반 충실 재구현을 `hfrsda_real` variant로 추가 — DualAttn-TB 대조군과 별개. NLA(ingredient neighbourhood 통합)+SLA(WHO-7 health 가중)+cosine 스코어링, emb_dim=64. 통일 평가 프로토콜·BPR 학습·graph health-score 사용 2가지 adaptation을 §4.1에 명시. §11.6 "health-aware baseline 부재" → "재구현이나 원 pipeline 아님"으로 갱신, 이제 논문 최대 공백 해소. **수치는 GPU 5-fold 실행 후 Table 1/B에 반영 예정**(scripts/run_hfrsda_real.bat). Local smoke(15ep/1fold): AUC=0.831 HR@10=0.630 NDCG@10=0.485. v2.0: **전면 재프레이밍**. 제목 교체: 증강 논지가 v1.3에서 철회되어 기존 제목("Why Graph Augmentation Fails")이 본문과 모순 상태였음 → "Auxiliary Nutritional Structure Substitutes for Interaction Data"로 변경, 발견 중심 프레임. **HFRS-DA 주장 전면 철회**: 기존 EXP-F/C의 "HFRS-DA는 구조적으로 고장났다" 주장은 자체 단순화 재구현(코드 docstring이 "Simplified re-implementation"이라 명시)에 근거했고 원저자 공식 구현이 공개돼 있어 방어 불가였음. 해당 모델을 **DualAttn-TB(자체 설계 topology-blind 대조군)**로 재정의 — 남의 논문 반박이 아니라 우리 모델의 그래프 사용 입증 도구가 되어 논지가 오히려 강화됨. Δ=0.000은 결함이 아니라 대조군의 설계된 성질이자 ablation 하네스 검증. §4.1에 귀속 고지 신설. Abstract/Contribution/Introduction/Research Gap/Conclusion(4) 전면 재작성 — 두 축(보조구조가 상호작용 대체 / 건강은 gradient 라우팅 문제) 중심. **§8.4 신설**: v2 vs v3 아키텍처-밀도 교차(v3는 100%에서 NDCG +19.5%지만 10%에서 −12.6%) — v2를 논문 본체로 확정. **§11 Limitations 신설**: 측정 함정 2건(HealthGain 저밀도 무효, 재현성 노이즈 바닥) 자진 보고 + health-aware 베이스라인 부재/단일 데이터셋/AUC 미해결 명시. Conclusion은 §12로 이동. v1.3: EXP-B 오표기 수정 — Table B의 "NutriGraphNet" 열이 실제로는 hfrsda 데이터였음; 실측 결과 진짜 NutriGraphNet이 더 우수(10% HR@10=0.738, NGCF 대비 +45.0%, 밀도 불변 0.99×).)

---

## Abstract

Health-aware food recommendation is most needed exactly where interaction data is hardest to obtain: clinical cohorts and national dietary surveys record only a handful of eating events per participant. We study this regime on **NutriGraph-KR**, a heterogeneous graph built from Korean national dietary survey data (20,820 users, 31,458 foods, 3,284 ingredients, 262,270 interactions; density 0.040%, ≈12.6 interactions per user — roughly ten times sparser than the MovieLens benchmarks on which modern graph recommenders were developed). We introduce **NutriGraphNet**, a dual-channel heterogeneous GAT that propagates over all nine edge types, including a `healthness` relation, under a health-constrained objective, and evaluate it against four published collaborative-filtering baselines, a re-implemented health-aware baseline (HFRS-DA), and a purpose-built topology-blind control under GPU 5-fold cross-validation.

**Auxiliary structure substitutes for interaction data — shown by two independent models.** Where interactions are scarce, the two models that consume auxiliary structure — NutriGraphNet and HFRS-DA — are essentially tied and both dominate the pure collaborative baselines: at 10% density HR@10=0.738 / 0.742 respectively, versus NGCF 0.509 (**+45%**), LightGCN 0.524, and SGL 0.088 (**8.4×**). Both are **density-invariant** (HR@10 scaling ratios 0.99× and 1.05× across a tenfold change in interaction volume), reaching ~95% of the best full-data score using one tenth of the interactions, whereas the collaborative baselines are data-hungry (MF 2.18×, SGL 4.06×). The property is two-sided: neither auxiliary model converts additional data into ranking quality, so NGCF overtakes both above ~50% density.

**The mechanism is verified, not inferred.** Removing all auxiliary edges costs NutriGraphNet **21.0%** HR@10 (removing `healthness` alone costs 7.2%), whereas our topology-blind control — identical in embedding capacity but structurally unable to read auxiliary edges — registers **Δ=0.000 exactly**, calibrating the ablation harness and isolating graph consumption as the source of the advantage. That control is itself a strong recommender (HR@10=0.656 at 10%, above every pure-CF baseline), so on ultra-sparse nutrition graphs propagation is not automatically worth its cost; NutriGraphNet's **+12.5%** margin over it at 10% density is what topology actually buys.

**Health constraints must be architecturally routed — NutriGraphNet's distinguishing property.** HealthGain@10 stays non-zero (≈−0.010) across four orders of magnitude of the health weight λ, with a robust plateau over λ∈[0.001, 0.1]; under full parameters the trade-off is functional and controllable (at λ≥0.5, HealthGain contracts toward zero while HR@10 falls 19.6–27.3%). HFRS-DA's health signal is fixed by contrast. This tunable health routing, not raw accuracy, is what sets NutriGraphNet apart: we report plainly that at full density HFRS-DA is the stronger ranker (NDCG@10 0.625 vs. 0.428), and that NutriGraphNet's collaborative encoder favours retrieval (HR) over fine ordering (NDCG) — a limitation five targeted architectural interventions did not close (Section 8.4).

**Negative and cautionary results.** SGL collapses at low density (HR@10=0.088 at 10%), but not because of augmentation: with augmentation disabled it still trails NGCF 2.18×, and the dropout sweep moves HR@10 by only 0.008 — within measured run-to-run noise. Architectural choices that improve full-density ranking actively harm the sparse regime (a rank-calibrated decoder gains 19.5% NDCG@10 at 100% yet loses 12.6% HR@10 at 10%). We document two measurement pitfalls that silently manufacture false conclusions in sparsity studies of health-aware recommenders.
...
3. **[Finding: auxiliary structure substitutes for interaction data]** Under GPU 5-fold CV, the two auxiliary-using models (NutriGraphNet and the re-implemented HFRS-DA) are tied and both dominate the pure-CF baselines wherever interactions are scarce (HR@10=0.738 / 0.742 at 10% density, +45% over NGCF); both are **density-invariant** (ratios 0.99× / 1.05× across a tenfold change), reaching ~95% of the best full-data score on one tenth of the interactions, while the CF baselines are data-hungry (MF 2.18×, SGL 4.06×). The converse holds with equal weight: neither converts extra data into ranking quality, and NGCF overtakes both above ~50% density.
...
| DualAttn-TB (control) | 0.8551 (±0.010) | 0.7200 | 0.6730 | 0.7340 | 0.8010 | 0.5977 | 0.5635 |
| HFRS-DA | **0.8796** (±0.001) | 0.8173 | 0.6740 | 0.7820 | 0.8350 | **0.6245** | 0.5814 |
| **NutriGraphNet** (λ=0.005) | 0.8620 (±0.006) | **0.7877** | 0.5660 | 0.7484 | 0.8252 | 0.4279 | 0.3378 |

*(Bold = best per column; GPU 5-fold CV; B_sparsity_100pct for baselines, HFRSDA_real_100pct for HFRS-DA, C_lambda_0.005/full for NutriGraphNet. F1 best is NutriGraphNet's; every other column is led by NGCF or HFRS-DA.)*

**Key observations from Table 1.** At full density NutriGraphNet is **not** the strongest model, and we report this plainly. **HFRS-DA leads on AUC (0.880), NDCG@10 (0.625) and MRR (0.581)**; **NGCF leads on HR@10 (0.784)**; NutriGraphNet leads only on F1 (0.788). NutriGraphNet's weakness is concentrated in ordering: its HR@10 (0.748) is competitive — it *retrieves* the relevant food into the top-10 — but its NDCG@10 (0.428) and MRR (0.338) are the lowest of the graph models, i.e. it *orders* the top-k poorly. Section 8.4 traces this to the collaborative encoder and shows it is not a health-loss artifact (λ=0.0 gives an even lower NDCG). The remaining observations:

1. **SGL anomaly.** SGL has moderate AUC (0.699) but collapsed HR@10 (0.358), **2.19× below NGCF**, foreshadowing the sparsity analysis (EXP-A/B).

2. **MF–SGL ranking paradox.** MF has the lowest AUC (0.547) yet the second-highest HR@10 (0.760) and a strong NDCG@10 (0.618) — AUC and top-K ranking decouple sharply on this graph.

3. **The two auxiliary-using models rank differently despite similar retrieval.** HFRS-DA and NutriGraphNet both consume ingredient/health structure and reach similar HR@10 (0.782 / 0.748), but HFRS-DA's shallow, identity-preserving representation orders far better (NDCG 0.625 vs 0.428). This retrieval-vs-ordering split, not overall dominance, is NutriGraphNet's signature — and motivates positioning it as a health-tunable low-density model rather than a general-purpose ranker (Sections 7, 10).

4. **The topology-blind control is competitive.** DualAttn-TB (AUC 0.855, HR@10 0.734, NDCG@10 0.598) beats NGCF on NDCG/MRR without reading a single auxiliary edge (EXP-F). On this data graph propagation must earn its cost rather than be assumed to pay.
...
| Density | MF | LightGCN | NGCF | SGL | DualAttn-TB | HFRS-DA | NutriGraphNet | Best |
|---------|-----|---------|------|-----|---------|---------|---------------|------|
| 10% | 0.349 | 0.524 | 0.509 | 0.088 | 0.656 | **0.742** | 0.738 (±0.018) | HFRS-DA |
| 30% | 0.617 | 0.690 | 0.701 | 0.212 | 0.734 | 0.743 | **0.755** (±0.009) | NutriGN |
| 50% | 0.721 | 0.723 | 0.748 | 0.272 | 0.727 | **0.772** | 0.742 (±0.006) | HFRS-DA |
| 70% | 0.744 | 0.729 | 0.763 | 0.332 | 0.737 | 0.745 | 0.722 (±0.020) | NGCF |
| 100% | 0.760 | 0.721 | **0.784** | 0.358 | 0.734 | 0.782 | 0.729 (±0.037) | NGCF |

*(HR@10; bold = best per row; see Figure 2. HFRS-DA and NutriGraphNet — the two auxiliary-using models — are within noise of each other at 10%/30% and both far above the pure-CF baselines.)*

**Table B-AUC. AUC vs. Interaction Density (GPU 5-fold CV)**

| Density | MF | LightGCN | NGCF | SGL | DualAttn-TB | HFRS-DA | NutriGraphNet |
|---------|-----|---------|------|-----|---------|---------|-------------------|
| 10% | 0.514 | 0.770 | 0.764 | 0.502 | 0.817 | **0.939** | 0.932 |
| 30% | 0.539 | 0.832 | 0.835 | 0.599 | 0.853 | **0.912** | 0.897 |
| 50% | 0.534 | 0.845 | 0.864 | 0.645 | 0.857 | **0.897** | 0.880 |
| 70% | 0.538 | 0.840 | 0.872 | 0.673 | 0.862 | **0.888** | 0.864 |
| 100% | 0.547 | 0.822 | 0.878 | 0.699 | 0.855 | **0.880** | 0.860 |

**Table B-NDCG. NDCG@10 vs. Interaction Density (GPU 5-fold CV)**

| Density | MF | LightGCN | NGCF | SGL | DualAttn-TB | HFRS-DA | NutriGraphNet |
|---------|-----|---------|------|-----|---------|---------|---------------|
| 10% | 0.236 | 0.339 | 0.338 | 0.042 | 0.542 | **0.567** | 0.508 |
| 30% | 0.461 | 0.482 | 0.482 | 0.109 | 0.588 | **0.586** | 0.505 |
| 50% | 0.576 | 0.517 | 0.548 | 0.157 | 0.589 | **0.613** | 0.504 |
| 70% | 0.593 | 0.509 | 0.522 | 0.200 | 0.599 | **0.601** | 0.409 |
| 100% | 0.618 | 0.499 | 0.557 | 0.228 | 0.598 | **0.625** | 0.411 |
...
**Finding B2 — Two Auxiliary-Using Models Dominate Where Interactions Are Scarce.** At 10% density the two models that consume auxiliary structure — HFRS-DA (0.742) and NutriGraphNet (0.738) — are essentially tied (Δ=0.004, within the ~0.005 reproducibility floor) and both tower over the pure collaborative baselines: **+45–46%** over NGCF (0.509), **+41–42%** over LightGCN (0.524), **8.4×** over SGL (0.088). The same holds at 30% (0.743 / 0.755 vs. NGCF 0.701). The advantage is widest exactly where interaction data is scarcest — the signature the auxiliary-substitution hypothesis predicts: when `eats` edges are decimated, the intact ingredient/food-similar/time structure still carries usable signal, and *both* heterogeneous models exploit it.

That this is *caused* by auxiliary-edge consumption, not mere correlation, is pinned down by the control. DualAttn-TB, whose forward pass never reads auxiliary edges (EXP-F: Δ HR@10 = 0.000 exactly for every removal), reaches only 0.656 at 10% — whereas NutriGraphNet, which demonstrably consumes them (EXP-F v2a: −21.0% HR@10 when all auxiliary edges are removed), reaches 0.738, **+12.5%** over that topology-blind reference. Two heterogeneous models beating the pure-CF and topology-blind models by the largest margin precisely where auxiliary structure is the only structure left is the paper's central result.

**Finding B3 — Auxiliary Structure Confers Density-Invariance.** Ranking models by HR@10 scaling ratio (10%→100%):
- **NutriGraphNet: 0.738→0.729, ratio=0.99×** and **HFRS-DA: 0.742→0.782, ratio=1.05×** — the two auxiliary-using models are essentially flat across a 10× change in interaction volume
...
Density-invariance is thus a property of *auxiliary-structure consumption*, shared by two independent architectures, not a quirk of one model. At 10% density both reach ~95% of the best full-data HR@10 any model achieves (NGCF's 0.784) using one tenth of the interactions — the operationally relevant property for domains where interaction data is expensive (clinical cohorts, national dietary surveys).

**Finding B4 — The Flip Side: the Auxiliary-Using Models Do Not Exploit Additional Data.** Density-invariance cuts both ways. NGCF converts extra interactions into ranking quality and overtakes both auxiliary models above ~50% density (at 100%: NGCF 0.784 vs. HFRS-DA 0.782 vs. NutriGraphNet 0.729). NutriGraphNet in particular plateaus at HR@10≈0.72–0.76 throughout; its own 0.722–0.755 variation is within its fold spread (σ up to 0.037) and the reproducibility floor (ΔHR@10≈0.002, ΔAUC≈0.005), so the apparent 50→100% dip is a plateau, not a decline. Both auxiliary-using models are therefore best read as **sparse-data specialists**: preferred below ~50% density, matched or beaten by NGCF above it. Between the two, HFRS-DA is the stronger ranker at every density (Table B-NDCG); NutriGraphNet's distinguishing property is not raw accuracy but tunable health routing (EXP-C), taken up in Section 7.
...
**The AUC trend is shared by both auxiliary-using models.** NutriGraphNet's AUC *decreases* with density (0.932→0.860), and so does HFRS-DA's (0.939→0.880) — while every pure-CF baseline *increases* (NGCF 0.764→0.878). That the two auxiliary-structure models move together, opposite to the CF models, supports the mechanistic reading: the auxiliary graph is held at full size while `eats` edges are subsampled, so at low density the discriminative signal comes overwhelmingly from intact auxiliary structure against a smaller, easier positive set. The effect is consistent across folds (σ≤0.008), not noise. We still treat the absolute AUC column cautiously (it is inflated by the fixed auxiliary graph at low density), but its direction is now explained rather than anomalous.
...
**Finding H2 — NutriGraphNet's Ordering Gap Resists Targeted Repair.** HFRS-DA's full-density ranking advantage (NDCG@10 0.625 vs. NutriGraphNet's 0.428) invites the question of whether the gap is a tunable defect or a structural property of the collaborative encoder. We ran five targeted interventions, each holding the 5-fold protocol, seed, and parameters fixed, and each falling short of HFRS-DA's NDCG at 100% density:

| Intervention | Rationale | 100% NDCG@10 |
|---|---|---|
| (baseline v2, HybridDecoder) | — | 0.411 |
| Rank-optimal cosine decoder | HybridDecoder miscalibrates order | 0.437 |
| Health-weight reduction / removal | health loss might starve ranking | ≤0.44 (unchanged; health is ~4% of loss at 100%) |
| Shallower encoder (L=1, L=2) | over-smoothing (cf. EXP-G) | 0.45 / 0.31 (worse, unstable) |
| InfoNCE contrastive term removed | cl loss never converges | 0.383 (worse) |
| Content-anchor branch (nutrition + ingredient identity, residual-fused) | preserve food identity | 0.408 |
| *HFRS-DA (target)* | — | *0.625* |

A checkpoint diagnostic explains the persistence: NutriGraphNet's food embeddings occupy ~11 of 64 effective dimensions after message passing, versus ~20 for HFRS-DA — the collaborative encoder compresses food-discriminative signal into a low-dimensional subspace, which supports retrieval (foods land in the top-10) but not fine ordering (they are not ranked near the top). Notably the interventions that improve full-density NDCG (cosine decoder) do **not** help at low density, and the content-anchor variant, while not fixing NDCG, is the *most stable* low-density model (10% HR@10=0.743±0.010, tying HFRS-DA). We report this negative result deliberately: it establishes that the retrieval-vs-ordering split is a structural property of collaborative message passing on ultra-sparse graphs, not a hyperparameter oversight, and it delimits where a health-tunable heterogeneous model like NutriGraphNet is and is not the right tool.
...
**11.6 The health-aware baseline is a re-implementation, and it is the stronger ranker.** We compare against HFRS-DA [Forouzandeh et al., 2024], re-implemented from the authors' official code (§4.1). Two honesty caveats. First, HFRS-DA's native recommendation and evaluation (user–user cosine CF, Precision/Recall/F1 over a user split) are not comparable to our unified sampled-ranking protocol, so our figures reflect its architecture and health mechanism under our evaluation rather than a verbatim re-run, and its health signal is our WHO-7 `healthness` edge rather than re-derived raw-gram thresholds. Second — and importantly for interpretation — under our own protocol HFRS-DA **outperforms our proposed NutriGraphNet** on AUC, NDCG@10 and MRR at every density (Tables 1, B). We do not present NutriGraphNet as the state of the art; its contribution is the tunable health-gradient routing HFRS-DA lacks (EXP-C) and its competitive, most-stable behaviour in the sparse regime (Finding H2). A comparison against further health-aware systems (SCHGN, FRMADHG, MOPI-HFRS) remains outstanding.
...
**(2) Auxiliary Structure Substitutes for Interaction Data.** Under GPU 5-fold cross-validation, the two models that consume auxiliary structure — NutriGraphNet and the re-implemented HFRS-DA — are tied at low density (HR@10 0.738 / 0.742 at 10%) and both dominate the pure-CF baselines (+45% over NGCF, 8.4× over SGL). Both are **density-invariant** (scaling ratios 0.99× / 1.05×), reaching ~95% of the best full-data score using one tenth of the interactions, while the CF baselines are data-hungry (MF 2.18×, SGL 4.06×). The mechanism is confirmed rather than inferred: removing the auxiliary edges costs NutriGraphNet 21.0% HR@10 (EXP-F v2a), while the topology-blind control is provably indifferent (Δ=0.000). The property is two-sided — neither auxiliary model exploits extra data, so NGCF overtakes both above ~50% density — and between the two, HFRS-DA is the stronger ranker at every density (NDCG@10 0.625 vs. NutriGraphNet's 0.428 at 100%). NutriGraphNet's collaborative encoder favours retrieval over fine ordering, a structural limitation that five targeted interventions did not close (Section 8.4). Both are best positioned as **sparse-data specialists**, not general-purpose rankers.
